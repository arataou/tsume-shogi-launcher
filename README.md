...
一个面向 5 / 7 / 9 手诘将棋的 Windows 便携式练习程序。界面聚焦诘将棋训练、题库、应手引擎、统计、成就和设置，不依赖 Shogi Explorer 或 ShogiHome。

## 功能

- 内置 1084 道题：精选集 457 题，扩展练习集 627 题；
- 攻方任意合法王手即可继续，不比较用户着法和题库首着；到达目标手数时由页面规则即时确认当前局面确实无玉方应手；严格手数只统计被接受的着法，错误尝试和悔棋不消耗次数；
- YaneuraOu 常驻提供快速玉方应手；服务启动阶段会预热常驻引擎，异常时使用题库回退应手；
- 自动检查合法走法、王手、打入、二歩、升变和玉方自王手；
- 按顺序刷题（默认）或随机刷题，支持上一题/下一题、严格手数、悔棋、重开、跳过、提示、答案、标记题/错题重刷；诘棋通关后仍可悔回最后一组着手；
- 成功后在棋盘区域显示轻量通关庆祝卡，失败后自动重开；
- 选中、落子、应手、错误、提示、悔棋和通关的轻量音效与 UI 动效，音效可在设置中关闭；
- 训练记录、连续天数、最快成绩、成就，以及今日 / 本周 / 本月 / 历史统计与活动记录；
- 现代蓝灰桌面界面、浅色棋盘、系统无衬线字体、外沿坐标、标准星位，以及统一的棋子与持駒牌面；
- 核心训练完全离线运行；另提供可选的在线精选链接，不包含剪贴板导入、KIF 导入或普通棋局复盘。
...
源码仓库不提交约 163 MB 的 `TsumeLauncher.exe`，该文件随 Windows Release 压缩包发布；这样可以避开 GitHub 普通文件的 100 MB 限制。只克隆源码时需要先按下方说明重新发布宿主程序。

旧版目录中如果仍有 `Start.bat`，它只是历史兼容脚本，不是当前启动流程的必需入口；正式流程直接双击 `TsumeLauncher.exe`。
