...
`TsumeLauncher.exe` 是正式启动入口，直接负责打开独立窗口。本版本不再提供浏览器回退入口；如果缺少 WebView2 Runtime，程序应明确提示安装该运行时。`Start.bat` 属于历史兼容脚本，不是当前流程的必需入口。
...
- 现代蓝灰训练主题、浅色棋盘、系统无衬线字体、外沿坐标、实体棋子和统一牌面持駒；
- 合法走法和王手即时拦截；
- 常驻 YaneuraOu 启动预热、应手中/就绪/回退状态、自动玉方应手和安全回退；
- 任意合法王手都可进入应手流程；引擎不承担正解首着比较，目标手数处只确认当前分支确实没有玉方合法应手；
...
- 计时、最快成绩、连续天数、进度和成就。
- 统计页提供今日、本周、本月和历史累计的完成、练习、错误、重开、完成率和平均用时，并展示所选周期的最近活动；
- 棋盘坐标放在外沿，加入四个标准星位，棋子和界面字体使用系统无衬线字体回退；
- 棋盘下方提供轻量的“等待玉方应手”状态动效，落子、反馈、统计数字和通关卡片使用短促动效，并响应系统 `prefers-reduced-motion` 偏好；
- 选中、落子、玉方应手、错误、提示、悔棋、重开、棋谱回放和通关使用浏览器内置 Web Audio 合成的低音量反馈音效，设置中可关闭，不增加外部资源依赖；

### P0：PC 版完成前还值得做（非阻塞）

当前版本已经补上启动预热状态、当前进度文字、非法着法分类、窄屏持駒牌、现代蓝灰棋盘、周期统计、等待应手动效和可选音效。剩下的精细项可以在实际使用一轮后按感受调整：

1. 引擎失败时提供“重试一次”按钮，但默认继续训练；
2. 记录最近一次实际引擎分支，答案页标注“题库参考手顺 / 本次引擎手顺”；
3. 视窗口高度把当前进度文字升级成小型完成进度环，避免增加桌面信息密度。
...
- 没有形成王手的着法即时拦截，不让引擎承担明显的 UI 规则提示；
